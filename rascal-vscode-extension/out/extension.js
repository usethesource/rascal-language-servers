...
exports.deactivate = exports.activate = exports.getRascalExtensionDeploymode = void 0;
...
let contentPanels = [];
let deployMode = true;
function getRascalExtensionDeploymode() {
    return deployMode;
}
exports.getRascalExtensionDeploymode = getRascalExtensionDeploymode;
function activate(context) {
    const serverOptions = deployMode
        ...
        : () => tryOpenConnection(serverPort, 'localhost', 10, 1000).then(s => ({
            writer: s,
            reader: s
        }));
    const clientOptions = {
    ...
    const client = new vscode_languageclient_1.LanguageClient('rascalmpl', 'Rascal MPL Language Server', serverOptions, clientOptions);
...
function activateTerminal(context) {
...
function registerContentViewSupport() {
    vscode.window.registerTerminalLinkProvider({
        provideTerminalLinks: (context, token) => {
            var pattern = new RegExp('Serving \'(?<theTitle>[^\']+)\' at \\|http://localhost:(?<thePort>[0-9]+)/\\|');
            ...
            if (result !== null) {
                ...
                return [
                    {
                        startIndex: matchAt,
                        length: result === null || result === void 0 ? void 0 : result.input.length,
        ...
        handleTerminalLink: (link) => {
            ...
            let url = theLink.url;
            let oldPanel = contentPanels.find(p => p.title === theLink.contentId);
            if (oldPanel === undefined) {
                const panel = vscode.window.createWebviewPanel(theLink.contentType, theLink.contentId, vscode.ViewColumn.One, {
                    enableScripts: true,
                });
...
function startJavaServerProcess(port, extensionPath) {
    return new Promise((started, failed) => {
        const classPath = path.join(extensionPath, 'dist', 'rascal-lsp-' + version + '.jar');
        const args = ['-cp', classPath, 'org.rascalmpl.vscode.lsp.RascalLanguageServer', '-port', '' + port];
...
function tryOpenConnection(port, host, maxTries, retryDelay) {
